...
// Rearranged TabType order to match the new desired order for the tab bar
type TabType = "overview" | "inflows" | "outflows" | "transfers" | "donations" | "proposals";
...
const TreasuryExplorer = () => {
  ...
  const treasuryData = {
    totalTreasury: {
      ...
      breakdown: [
        ...
        { icon: "USDM", amount: "≈ 7.26M", currency: "USDM" },
        { icon: "USDA", amount: "≈ 7.26M", currency: "USDA" },
      ]
    },
    treasuryRequesting: {
      confirming: "$0",
    ...
    assets: {
      ...
      breakdown: [
        { label: "Address #1 > #2 >", items: [
          ...
          { icon: "USDM", amount: "USDM ≈ 5.76M" },
          { icon: "USDAda", amount: "USDA ≈ 5.7M" }
        ]}
      ]
    },
    loans: {
  ...
  // New tab order matching the instructions:
  // Overview | Inflows | Outflows | Transfers | Donations | Proposals
  const tabs: Array<{ id: TabType; label: string; count?: number }> = [
    { id: "overview", label: "Overview" },
    { id: "proposals", label: "Proposals", count: tabCounts.proposals },
    { id: "outflows", label: "Outflows", count: tabCounts.outflows },
    { id: "inflows", label: "Inflows", count: tabCounts.inflows },
    { id: "transfers", label: "Transfers", count: tabCounts.transfers },
    { id: "donations", label: "Donations", count: tabCounts.donations },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 font-semibold">Complete treasury balance and asset breakdown</p>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Clock className="w-4 h-4" />
                  <span>Updated {treasuryData.totalTreasury.lastUpdated}</span>
                </div>
              </div>
              ...
              <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                <CardHeader>
                  <CardTitle className="text-foreground">Total Treasury</CardTitle>
                  <CardDescription className="font-semibold">Current treasury balance across all assets</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <p className="text-4xl font-bold text-foreground mb-1">{treasuryData.totalTreasury.ada} ADA</p>
                      <p className="text-xl font-semibold text-muted-foreground">≈ $46.47M</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-border">
                    {treasuryData.totalTreasury.breakdown.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                        {getCurrencyIcon(item.icon)}
                        <div>
                          <p className="font-semibold text-foreground">{item.amount}</p>
                          <p className="text-xs font-semibold text-muted-foreground">{item.currency}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                <CardHeader>
                  <CardTitle className="text-foreground">Treasury Requesting</CardTitle>
                  <CardDescription className="font-semibold">Pending and confirming requests</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
                      <p className="text-sm font-semibold text-muted-foreground mb-1">Confirming</p>
                      <p className="text-2xl font-bold text-foreground">{treasuryData.treasuryRequesting.confirming}</p>
                    </div>
                    <div className="p-4 bg-blue-500/10 rounded-lg border border-blue-500/20">
                      <p className="text-sm font-semibold text-muted-foreground mb-1">Requesting</p>
                      <p className="text-2xl font-bold text-foreground">{treasuryData.treasuryRequesting.requesting}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                <CardHeader>
                  <CardTitle className="text-foreground">Assets</CardTitle>
                  <CardDescription className="font-semibold">Total: {treasuryData.assets.total}</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {treasuryData.assets.breakdown.map((asset, idx) => (
                      <div key={idx} className="p-4 bg-muted/30 rounded-lg">
                        <p className="text-sm font-semibold text-muted-foreground mb-3">{asset.label}</p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          {asset.items.map((item, itemIdx) => (
                ...
                <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-foreground">Bounties</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold text-foreground mb-2">10</p>
                    <p className="text-sm font-semibold text-muted-foreground">10 active transfers</p>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-foreground">Outflows</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold text-foreground mb-2">10</p>
                    <div className="mt-3 space-y-2">
                      <span className="text-xs font-semibold text-muted-foreground">10 active transfers</span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-foreground">Ambassador</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold text-foreground mb-2">10</p>
                    <div className="mt-3 space-y-2">
                      <div className="flex justify-center items-center gap-2 text-sm">
                          <span className="font-medium text-foreground">10 active transfers</span>
                        </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-foreground">Loans</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold text-foreground">{treasuryData.loans.total}</p>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-foreground">Donations</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
              ...
              <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                <CardHeader>
                  ...
                  <CardDescription className="font-semibold">At-a-glance treasury metrics</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-muted/30 rounded-lg">
                      <p className="text-xs font-semibold text-muted-foreground mb-1">Active Projects</p>
                      <p className="text-xl font-bold text-foreground">{projects.length}</p>
                    </div>
                    <div className="text-center p-4 bg-muted/30 rounded-lg">
                      <p className="text-xs font-semibold text-muted-foreground mb-1">Total Outflows</p>
                      <p className="text-xl font-bold text-foreground">{tabCounts.outflows}</p>
                    </div>
                    <div className="text-center p-4 bg-muted/30 rounded-lg">
                      <p className="text-xs font-semibold text-muted-foreground mb-1">Total Inflows</p>
                      <p className="text-xl font-bold text-foreground">{tabCounts.inflows}</p>
                    </div>
                    <div className="text-center p-4 bg-muted/30 rounded-lg">
                      <p className="text-xs font-semibold text-muted-foreground mb-1">Active Proposals</p>
                      <p className="text-xl font-bold text-foreground">{tabCounts.proposals}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          {/* Inflows Tab */}
          {activeTab === "inflows" && (
            // The previous "Transfers Tab - Inflows Explorer" section moved up here (was last).
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
          ...
          {/* Outflows Tab */}
          {activeTab === "outflows" && (
            // The previous "Applications Tab - Project-Level Explorer" (was called outflows)
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
          ...
          {activeTab === "transfers" && (
            <>
              {/* The previous "Spends Tab - Outflows Explorer" content moved here (was inflows) */}
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
              ...
              {/* Also, preserve the original bounties card */}
              <Card className="border-border/60 bg-card/40 backdrop-blur-lg">
                <CardContent className="p-8">
                  <div className="text-center">
                    <Award className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
                    <h3 className="text-xl font-bold text-foreground mb-2">Bounties</h3>
                    <p className="text-muted-foreground font-semibold">{tabCounts.transfers} active transfers</p>
                  </div>
                </CardContent>
              </Card>
            </>
